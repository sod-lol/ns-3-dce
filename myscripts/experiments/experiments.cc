...
void
Finished (int *pstatus, uint16_t pid, int status)
{      
  Simulator::Stop ();
}
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue("fileSize", "File size in megabyte", file_size);
  cmd.AddValue("distance", "Distance from nodes in meter", distance);
  ...
  Config::SetDefault ("ns3::MmWavePointToPointEpcHelper::S1apLinkDelay", TimeValue (MicroSeconds (12000)));

  Config::SetDefault ("ns3::MmWavePointToPointEpcHelper::S1uLinkDelay", TimeValue (MilliSeconds (10)));
  Config::SetDefault ("ns3::PointToPointEpcHelper::S1uLinkDelay", TimeValue (MilliSeconds (10)));
  Config::SetDefault ("ns3::PointToPointEpcHelper::S1apLinkDelay", TimeValue (MicroSeconds (12000)));
  ...
  //mmWaveHelper->SetPathlossModelAttribute ("Frequency", DoubleValue (28e9));
...
/*  mmWaveEpcHelper->SetAttribute("UeBaseAddress", StringValue("7.0.0.0"));
  ...
  //lteHelper_2->SetPathlossModelType ("ns3::AlwaysLosChannelConditionModel");
  ...
  lteHelper_2->SetAttribute ("PathlossModel", StringValue ("ns3::FriisPropagationLossModel"));
  //lteHelper_2->SetPathlossModelAttribute ("Frequency", DoubleValue (28e9));
  //lteHelper_2->AddPropagationLoss ("ns3::RangePropagationLossModel",
  //                                "MaxRange", DoubleValue (200.0));
  ...
  {
      setPos (nodes.Get (i), distance, 0, 1);
...
*/
    ...
    dce.AddArgument ("0.3");
    dce.AddArgument ("--num");
    dce.AddArgument (file_size + "M");
    dce.AddArgument ("-f");
    dce.AddArgument ("m");
    dce.SetFinishedCallback (MakeBoundCallback (&Finished, &status));
    //dce.AddArgument ("-e");
    //dce.AddArgument ("-e");
    //dce.AddArgument ("-l");
    //dce.AddArgument ("128K");
    // dce.AddArgument ("-w");
    // dce.AddArgument (std::to_string( std::stoi(file_size) / 2) + "M");
